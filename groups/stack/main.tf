...
locals {
  ...
  stack_name       = "deltas-service"
  stack_fullname   = "${local.stack_name}-stack"
  name_prefix      = "${local.stack_name}-${var.environment}"

  public_lb_cidrs  = ["0.0.0.0/0"]
  lb_subnet_ids    = "${var.delta_sync_lb_internal ? local.application_ids : local.public_ids}" # place ALB in correct subnets
  lb_access_cidrs  = "${var.delta_sync_lb_internal ?
  ...
  app_access_cidrs = "${var.delta_sync_lb_internal ?
                      concat(local.internal_cidrs,local.vpn_cidrs,local.management_private_subnet_cidrs,split(",",local.application_cidrs)) :
...
module "ecs-stack" {
  ...
  delta_sync_lb_internal    = var.delta_sync_lb_internal
}
...
module "ecs-services" {
  ...
  # insolvecy data api variables
  insolvency_data_api_release_version       = var.insolvency_data_api_release_version
  ida_application_port      = "10000" // ??

  # insolvecy delta consumer  variables
  insolvency_delta_consumer_release_version = var.insolvency_delta_consumer_release_version
  insolvency_delta_consumer_application_port = "11000" // ??
  insolvency_delta_consumer_desired_count   = var.insolvency_delta_consumer_desired_count
